...
const { encrypt, decrypt, maskSecret, isMaskedValue } = require('./src/utils/encryption');
...
async function runTests() {
  ...
  const { SystemSettings, Shop, User } = require('./src/models');
  ...
  const [user] = await User.findOrCreate({
    where: { email: 'admin_test_sprint1@zana.com' },
    defaults: {
      name: 'Admin Tester',
  ...
  const mockReq = (body = {}) => ({
    shopId: testShopId,
    user: { id: testUserId, name: 'Admin Tester' },
    body
  });
  ...
  assert.strictEqual(parseFloat(getData?.taxRate), 16.50, 'GET should return taxRate 16.50');
