...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    // productId as INTEGER (matching Products.id type) — no FK to avoid type mismatch
    ...
    if (dialect === 'mysql' || dialect === 'mariadb') {
      try {
        ...
        console.log('Modify saleStatus ENUM skipped:', err.message);
      }
    } else if (dialect === 'postgres') {
      try {
        await queryInterface.sequelize.query(`ALTER TYPE "enum_Sales_saleStatus" ADD VALUE 'partial_refund';`);
      } catch (err) { /* Ignore if value already exists */ }
    }
  },

  down: async (queryInterface, Sequelize) => {
    const tableInfo = await queryInterface.describeTable('SaleRefunds');
    if (tableInfo.productId) await queryInterface.removeColumn('SaleRefunds', 'productId');
    if (tableInfo.quantity) await queryInterface.removeColumn('SaleRefunds', 'quantity');
    if (tableInfo.refundAmount) await queryInterface.removeColumn('SaleRefunds', 'refundAmount');
    if (tableInfo.refundedBy) await queryInterface.removeColumn('SaleRefunds', 'refundedBy');
    if (tableInfo.refundedAt) await queryInterface.removeColumn('SaleRefunds', 'refundedAt');
  }
};
