...
- **[BACKLOG-04] Forecast Cache Invalidation on Delayed Payment Settlement**: The backend `forecastCache` stores model projections with a 1-hour TTL keyed on the SHA-256 hash of `dates` and `values` retrieved from `GET /api/insights/daily-sales`. Because `getDailySales` filters strictly on `saleStatus = 'completed'`, any asynchronous transaction (e.g. M-Pesa STK push, card authorization, credit sale) initiated in `pending` status is excluded from the aggregated daily revenue. If a merchant or system requests a forecast while the transaction is `pending`, a forecast is generated and cached. When the payment callback subsequently settles the sale to `completed`, the database updates, but the cached forecast continues to be served until the 1-hour TTL expires or a subsequent distinct data hash is generated. Future work should invalidate the shop's forecast cache (`forecast:${shopId}:*`) inside the payment settlement / webhook handler (e.g. in `saleController.js` or `mpesa.js`) upon transition to `completed`.
